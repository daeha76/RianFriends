...
public sealed class HungerIncreaseJob : BackgroundService
{
    ...
    private async Task ProcessHungerIncreaseAsync(CancellationToken ct)
    {
        ...
        var totalCount = await context.Avatars.CountAsync(ct);
        _logger.LogInformation("배고픔 처리 대상 아바타: {Count}개", totalCount);
        ...
        for (var offset = 0; offset < totalCount; offset += BatchSize)
        {
            ...
            var friendIds = avatars.Select(a => a.FriendId).ToList();
            ...
            foreach (var avatar in avatars)
            {
                ...
                var result = avatar.IncreaseHunger(HungerIncreaseAmount);

                if (result.IsFailure)
                {
                    _logger.LogWarning(
                        "배고픔 증가 실패: AvatarId={AvatarId}, Error={Error}",
                        avatar.Id, result.Error);
                    continue;
                }
